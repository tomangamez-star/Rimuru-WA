...
function contextInfo(m){const x=m?.message||{};return x.extendedTextMessage?.contextInfo||x.imageMessage?.contextInfo||x.videoMessage?.contextInfo||x.documentMessage?.contextInfo||{}}
function candidates(m,c){
 const ci=contextInfo(m),mentions=[...(ci.mentionedJid||[]),...(c?.mentionedJid||[])]
...
function amountFrom(args=[]){for(const a of args){const n=Number(String(a).replace(/[,@]/g,''));if(Number.isFinite(n)&&n>=0)return Math.floor(n)}return null}
async function targetFor(m,c,self){
 const xs=candidates(m,c),hit=await registeredId(xs)
 if(hit)return hit
...
async function admin(sock,m,c){
 ...
 const target=await targetFor(m,c,self)
 ...
 const amount=amountFrom(c.args)
