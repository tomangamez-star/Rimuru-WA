...
function createUi({economy,casino,sendButtons,logger}){const send=(s,j,m,t)=>s.sendMessage(j,{text:t},{quoted:m}),P=m=>({id:canonicalUserId(m.key),name:displayName(m),jid:m.key.remoteJid}),rpg=createRpg({economy,logger,sendButtons}),aiChats=new Map(),masterChats=new Set()
 ...
 async function aiCommand(s,m,c){const p=P(m),x=String(c.args[0]||'').toLowerCase();if(x==='on'){aiChats.set(p.jid,true);await send(s,p.jid,m,'🤖 *RIMURU AI: ON*\n\nDM me normally. In groups, mention *Rimuru* or reply to me.');return true}if(x==='off'){aiChats.delete(p.jid);masterChats.delete(p.id);await send(s,p.jid,m,'🌙 *RIMURU AI: OFF*');return true}await send(s,p.jid,m,`🤖 AI: *${aiChats.get(p.jid)?'ON':'OFF'}*\nModel: *${ai.model}*\n\n/ai on • /ai off`);return true}
 async function startMasterTalk(s,m){const p=P(m),player=await rpg.getPlayer(p.id);if(!player?.currentMaster){await send(s,p.jid,m,'🧙 Choose a Master first.');return true}masterChats.add(p.id);aiChats.set(p.jid,true);await send(s,p.jid,m,`💬 *MASTER LINK OPEN*\n\n> 🧙 *${player.currentMaster}*\n\nChat normally. Use */endtalk* to return to Rimuru.`);return true}
 async function maybeAi(s,m,content){const p=P(m),text=String(content.conversation||content.extendedTextMessage?.text||'').trim();if(!text||text.startsWith('/')||!aiChats.get(p.jid)||m.key.fromMe)return false;const group=p.jid.endsWith('@g.us'),ctx=content.extendedTextMessage?.contextInfo||{};if(group&&!ctx.quotedMessage&&!/\brimuru\b/i.test(text)&&!masterChats.has(p.id))return false;try{const player=masterChats.has(p.id)?await rpg.getPlayer(p.id):null,a=await ai.chat({jid:p.jid,userId:p.id,name:p.name,text,mode:masterChats.has(p.id)?'master':'rimuru',player});await send(s,p.jid,m,a||'...');return true}catch(e){logger.warn({err:e},'Groq AI failed');await send(s,p.jid,m,'⚠️ My communication link glitched. Game systems are still online.');return true}}
 ...
 async function section(s,m,id){
  ...
  if(id==='rpg_talk_master')return startMasterTalk(s,m)
 ...
 async function route(s,m,content){const c=commandParts(content);if(!c)return false;const p=P(m)
  ...
  if(c.command==='/ai')return aiCommand(s,m,c)
  if(c.command==='/talk')return startMasterTalk(s,m)
  if(c.command==='/endtalk'){masterChats.delete(p.id);await send(s,p.jid,m,'🌊 Master link closed. You’re talking to Rimuru again.');return true}
 ...
 return{route,section,menu,balance,leaderboard,profile,maybeAi}}
