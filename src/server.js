...
class WhatsAppConnection {
  constructor () {
    ...
    this.reconnectAttempt=0; this.pairingReady=false; this.pairingInFlight=false; this.stopping=false; this.seen=new Map(); this.minesGames=new Map()
  }
  ...
  async connect () {
    ...
    sock.ev.on('messages.upsert',({messages,type})=>{if(type!=='notify')return;for(const message of messages)void this.handleMessage(sock,message).catch(async error=>{logger.error({err:error,jid:message?.key?.remoteJid},'WhatsApp command failed');const jid=message?.key?.remoteJid;if(jid)await sock.sendMessage(jid,{text:`Command failed: ${error.message}`},{quoted:message}).catch(()=>{})})})
